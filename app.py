...
st.write("Select filters to scan your ASX list efficiently and avoid timeouts.")
...
def get_asx_data_for_batch(asx_tickers, range_label):
    ...
    for idx, symbol in enumerate(asx_tickers):
        ...
        try:
            ...
            div_yield = info.get('dividendYield', None)  # Extract Dividend Yield decimal
            ...
            results.append({
                ...
                "P/E Ratio": pe_ratio,
                "P/B Ratio": pb_ratio,
                "Dividend Yield": div_yield,  # Store yield data
                "NPAT (TTM)": npat,
...
if os.path.exists(csv_path):
    ...
    raw_ticker_list = df_tickers[0].tolist()
        ...
        if not df_results.empty:
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Dividend Yield": st.column_config.NumberColumn("Dividend Yield", format="%.2f%%", help="Yield displayed as percentage. 1 = 100%"),
            ...
            # Multiply by 100 on the export CSV so the raw percentage format is standard inside Excel
